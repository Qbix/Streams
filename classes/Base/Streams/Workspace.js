/**
 * Autogenerated base class representing workspace rows
 * in the Streams database.
 *
 * Don't change this file, since it can be overwritten.
 * Instead, change the Streams/Workspace.js file.
 *
 * @module Streams
 */
...
/**
 * Base class representing 'Workspace' rows in the 'Streams' database
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Streams', 'indexes', 'Workspace'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{{dbname}}.')+'{{prefix}}workspace');
	} else {
		var conn = Db.getConnection('Streams');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'workspace';
 ...
 * @type string
 */
Base.prototype.className = "Streams_Workspace";
...
return [["varbinary","15","",false],false,"PRI",null];
};

/**
 * Method is called before setting the field and verifies if value is string of length within acceptable limit.
 * Optionally accept numeric value which is converted to string
 * @method beforeSet_parentName
 * @param {string} value
 * @return {string} The value
 * @throws {Error} An exception is thrown if 'value' is not string or is exceedingly long
 */
Base.prototype.beforeSet_parentName = function (value) {
		if (value == undefined) return value;
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number" && !(value instanceof Buffer))
			throw new Error('Must pass a String or Buffer to '+this.table()+".parentName");
		if (typeof value === "string" && value.length > 15)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".parentName");
		return value;
};

	/**
	 * Returns the maximum string length that can be assigned to the parentName field
	 * @return {integer}
	 */
Base.prototype.maxSize_parentName = function () {

		return 15;
};

	/**
	 * Returns schema information for parentName column
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
