 ...
 * Instead, change the Streams_Workspace.php file.
 *
 * @module Streams
 */
/**
 * Base class representing 'Workspace' rows in the 'Streams' database
 * @class Base_Streams_Workspace
 * @extends Db_Row
 *
...
abstract class Base_Streams_Workspace extends Db_Row
{
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Streams', 'indexes', 'Workspace', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'workspace');
		} else {
			$conn = Db::getConnection('Streams');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'workspace';
  			if (!$with_db_name)
  				return $table_name;
  ...
  array (
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Streams_Workspace'))
		);
	}
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_parentName($value)
	{
		if (!isset($value)) {
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_parentName()
	{
