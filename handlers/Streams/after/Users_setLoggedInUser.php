...
function Streams_after_Users_setLoggedInUser($params)
{
	...
	if ($token = Q::ifset($_SESSION, 'Streams', 'inviteFollowedToken', null)) {
		...
		if ($invite and !$invite->getExtra('dontAutoAccept')) {
			$stream = Streams_Stream::fetch($user->id, $invite->publisherId, $invite->streamName);
			if ($stream and !Streams_Stream::getConfigField($stream->type, array("invite", "dontAutoAccept"), false)) {
